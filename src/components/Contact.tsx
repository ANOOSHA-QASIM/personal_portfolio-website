...
import { FaLinkedinIn, FaInstagram, FaEnvelope, FaMapMarkerAlt, FaPhoneAlt, FaPaperPlane } from 'react-icons/fa'
...
const Contact = () => {
  const socialLinks = [
    ...
    { icon: <FaSquareXTwitter />, url: "https://x.com/anushainnovates" }
  ];
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="lg:col-span-7 bg-slate-800/40 p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] border border-white/20 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.6)] relative group"
          >
            ...
            <form onSubmit={onSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <input type="text" name="name" required placeholder="Full Name" className="w-full bg-white/[0.05] border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-teal/60 transition-all duration-300 placeholder:text-slate-400 text-sm" />
                <input type="email" name="email" required placeholder="Email Address" className="w-full bg-white/[0.05] border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-teal/60 transition-all duration-300 placeholder:text-slate-400 text-sm" />
              </div>
              <input type="text" name="subject" required placeholder="Project Subject" className="w-full bg-white/[0.05] border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-teal/60 transition-all duration-300 placeholder:text-slate-400 text-sm" />
              <textarea name="message" required rows={5} placeholder="Your Message" className="w-full bg-white/[0.05] border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-teal/60 transition-all duration-300 resize-none placeholder:text-slate-400 text-sm" />
              
              <div className="pt-4">
                <ProjectButton text={isSubmitting ? "Sending..." : "Send Message"} icon={<FaPaperPlane className="text-[10px]" />} type="submit" disabled={isSubmitting} className="w-full justify-center" />
                {result && (
          ...
          {/* Contact Info with Staggered Fade-In */}
          <motion.div 
            variants={staggerContainer}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
            className="lg:col-span-5 flex flex-col h-full space-y-4"
          >
            {/* Contact Cards */}
            <motion.div variants={fadeInUp} className="p-4 rounded-2xl bg-teal/10 border border-teal/20 transition-all duration-300 hover:bg-teal/20 hover:border-teal/40 hover:-translate-y-1 cursor-pointer group/card">
            ...
            <motion.div variants={fadeInUp} className="p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20 transition-all duration-300 hover:bg-blue-500/20 hover:border-blue-500/40 hover:-translate-y-1 cursor-pointer group/card">
                <div className="flex items-center gap-4">
            ...
            <motion.div variants={fadeInUp} className="p-4 rounded-2xl bg-purple-500/10 border border-purple-500/20 transition-all duration-300 hover:bg-purple-500/20 hover:border-purple-500/40 hover:-translate-y-1 cursor-pointer group/card">
                <div className="flex items-center gap-4">
            ...
            <motion.div variants={fadeInUp} className="flex justify-center lg:justify-start gap-3 pt-2">
            ...
            <motion.div variants={fadeInUp} className="mt-auto p-8 rounded-[2rem] md:rounded-[2.5rem] bg-gradient-to-br from-slate-800/50 to-transparent border border-white/10 relative overflow-hidden group shadow-2xl">
